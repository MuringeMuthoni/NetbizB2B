  ...
  divlogo:boolean=true;
    ...
    Get_users(){ 
      ...
      try {           
        ...
        this.mydb.db.executeSql('SELECT coluserid,colusername,colcourse  FROM userss ORDER BY rowid DESC', {})
           .then(res => {          
             if (res.rows.length > 0){      
          
                this.Wcf.User_id =   res.rows.item(0).coluserid;  
                this.Wcf.Username = res.rows.item(0).colusername;
                      
                console.log('user details: ' + this.Wcf.User_id + ", " + this.Wcf.Username   )
