...
export class LoginPage implements OnInit {
  ...
  ForgotPassword(){
    ...
    Save_user(feedback){
      ...
      this.Wcf.User_id = zote[0]; 
      this.Wcf.Username = zote[1];     
      console.log('registering user: ' + this.Wcf.User_id + ';' + this.Wcf.Username  )    
      try {
         this.mydb.db.executeSql('INSERT INTO userss VALUES (NULL, ?, ? , ? )', [ this.Wcf.User_id,this.Wcf.Username])  
      .then(res => {                 
            this.Wcf.user_log_status = "registered";           
            var data =this.Userid + ';7979'      
